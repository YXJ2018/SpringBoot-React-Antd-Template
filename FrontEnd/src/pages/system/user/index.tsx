...
const UserManage = memo(function UserManage() {
  ...
  const [allRoles, setAllRoles] = useState<{ key: string; title: string; disabled?: boolean }[]>([]);
    ...
    const result: ProColumnType<UserVO>[] = [
      ...
      },
      {
        ...
        fieldProps: {
          mode: 'multiple',
          placeholder: '请选择角色',
          options: allRoles?.map((r) => ({ value: Number(r.key), label: r.title, disabled: r.disabled })),
        },
